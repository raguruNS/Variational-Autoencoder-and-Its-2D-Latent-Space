...
Building and training a normal autoencoder

Visualizing the normal autoencoder's 2D latent space

Building and training a Variational Autoencoder (VAE)

Using reconstruction loss and KL divergence for VAE training
...
Measuring latent-space grouping using 5-nearest-neighbour classification

Generating a 15 × 15 decoded latent-space grid

Testing extreme latent points

Testing a point far outside the learned latent space

Comparing the normal autoencoder and VAE

Recording observations from the experiment
...
Latent range, dimension 1

-17.87 to 24.08

-3.92 to 3.71

Latent range, dimension 2

-17.78 to 13.45

-3.81 to 3.19

Average distance between digit centres
...
The VAE produces a more compact 2D latent representation.

Similar handwritten digits tend to form nearby groups.

Some digit classes are clearly separated, while others overlap.

The 2D restriction makes complete separation of all ten classes difficult.

The VAE latent space supports smooth transitions between generated digits.

The KL divergence encourages latent representations to stay around the standard normal distribution.

Reconstructions are recognizable but can be blurry.

Points moderately outside the learned latent region can still produce digit-like images.

Very distant latent points can produce unnatural or saturated patterns.

In this run, the VAE achieved a 5-NN accuracy of 0.799, compared with 0.773 for the normal autoencoder.
...
MNIST is downloaded/loaded.

The normal autoencoder is trained.
...
Latent spaces are plotted.

The decoded latent grid is generated.

Extreme latent-point experiments are performed.

Written observations are displayed.
